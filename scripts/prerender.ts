...
async function prerender() {
  ...
  for (const route of staticRoutes) {
    const html = injectHeadIntoHtml(templateHtml, route.meta);
  ...
  for (const post of samplePosts) {
    ...
    const postHtml = injectHeadIntoHtml(templateHtml, postMeta);
    writeRouteHtml(`/blog/${post.slug}`, postHtml);
    writeRouteHtml(`/post/${post.slug}`, postHtml);
  }

  console.log(`[Prerender Success] Successfully pre-rendered static HTML for ${staticRoutes.length + samplePosts.length * 2} routes!`);
}
