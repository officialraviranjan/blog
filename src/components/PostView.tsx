...
import { Calendar, Clock, ArrowLeft, ArrowRight, Share2, Link2, MessageSquare, CornerDownRight, Check, AlertTriangle, Lightbulb, Info } from 'lucide-react';
    ...
    lines.forEach((line, index) => {
      ...
      if (trimmed.startsWith('#')) {
        ...
        if (match) {
          ...
          if (level === 1) {
            renderedElements.push(
              <h1 key={`h1-${index}`} id={id} className="mt-8 mb-4 font-sans text-3xl font-bold tracking-tight text-slate-900 dark:text-white border-b border-slate-100 pb-2 dark:border-slate-800">
                {text}
              </h1>
            );
          } else if (level === 2) {
    ...
    <div className="relative w-full" id="blog-single-page">
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap gap-2 mb-4" id="post-hero-categories">
            {post.categories.map((cat) => (
              <button
                ...
                className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold uppercase tracking-wider text-blue-600 dark:bg-slate-800 dark:text-cyan-400"
              >
                {cat}
              </button>
            ))}
            ...
            <div className="mt-8 flex flex-wrap gap-2 border-t border-slate-100 pt-6 dark:border-slate-800" id="post-tags-row">
              {post.tags.map((tag) => (
                <button
                  ...
                  className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600 hover:bg-blue-50 hover:text-blue-600 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-cyan-400"
                >
                  #{tag}
                </button>
              ))}
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-slate-100 pt-8 dark:border-slate-800" id="post-navigation">
          {prevPost ? (
            <button
              onClick={() => onPostSelect(prevPost)}
              className="flex flex-col items-start rounded-2xl border border-slate-100 bg-white p-5 text-left transition-colors hover:bg-slate-5.0 dark:border-slate-800 dark:bg-slate-900 dark:hover:bg-slate-800/60"
              id="prev-post-nav"
            >
          ...
          {nextPost ? (
            <button
              onClick={() => onPostSelect(nextPost)}
              className="flex flex-col items-end rounded-2xl border border-slate-100 bg-white p-5 text-right transition-colors hover:bg-slate-5.0 dark:border-slate-800 dark:bg-slate-900 dark:hover:bg-slate-800/60"
              id="next-post-nav"
            >
              <span className="font-sans text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 flex items-center gap-1">
                Next Post
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {relatedPosts.map((p) => (
                <div
                  ...
                  className="cursor-pointer group flex gap-4 overflow-hidden rounded-xl border border-slate-100 bg-white p-4 shadow-xs hover:-translate-y-0.5 hover:shadow-sm dark:border-slate-800 dark:bg-slate-900 transition-all"
                >
                  <img
                    src={p.cover}
                    alt={p.title}
