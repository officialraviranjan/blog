...
export default function PostCard({ post, onCategoryClick }: PostCardProps) {
  return (
    <Link
      to={`/blog/${post.slug}`}
      className="group flex flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-xs transition-all duration-300 hover:-translate-y-1 hover:shadow-md dark:border-slate-800/80 dark:bg-slate-900"
        ...
        <div className="absolute top-3 left-3 flex flex-wrap gap-1" id="card-categories">
          {post.categories.map((cat) => (
            <button
              ...
              className="rounded-full bg-white/90 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-blue-600 shadow-xs hover:bg-blue-600 hover:text-white dark:bg-slate-900/90 dark:text-cyan-400 dark:hover:bg-cyan-500 dark:hover:text-slate-900 transition-colors cursor-pointer"
              id={`cat-badge-${cat}`}
            >
              {cat}
            </button>
