...
export default function AdSenseUnit({
  ...
  // Initialize the AdSense unit with consent check
  useEffect(() => {
    const initAd = () => {
      try {
        ...
        if (windowObj) {
          ...
          if (consent === 'declined') {
            (windowObj.adsbygoogle = windowObj.adsbygoogle || []).requestNonPersonalizedAds = 1;
          } else if (consent === 'accepted') {
            (windowObj.adsbygoogle = windowObj.adsbygoogle || []).requestNonPersonalizedAds = 0;
          }
    ...
    window.addEventListener('cookie-consent-updated', initAd);
    return () => {
      window.removeEventListener('cookie-consent-updated', initAd);
    };
  }, [slot, client]);
